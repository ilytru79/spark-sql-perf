...
object ModelBuilderSSP {
  ...
  def newNaiveBayesModel(pi: Vector, theta: Matrix): NaiveBayesModel = {
    val model = new NaiveBayesModel("naivebayes-uid", pi, theta, theta)
...
object TreeBuilder {
  ...
  def randomBalancedDecisionTree(
    ...
    val impurityCalculator = if (isRegression) {
      ImpurityCalculator.getCalculator("variance", Array.fill[Double](3)(0.0),0)
    } else {
      ImpurityCalculator.getCalculator("gini", Array.fill[Double](labelType)(0.0),0)
    }
